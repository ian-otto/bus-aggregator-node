...
class BusNotifier extends EventEmitter {
    ...
    add_watcher(watched_stop) {
        if(!this._watching[watched_stop.stop_id]) {
            this._watching[watched_stop.stop_id] = [];
        }
        this._watching[watched_stop.stop_id].push(watched_stop);
    ...
    _add_watch_process(watched_stop) {
        if(!this._watch_processes[watched_stop.stop_id]) {
            this._watch_processes[watched_stop.stop_id] = setInterval(this._execute_stop_update.bind(this), 10000, watched_stop);
            this._execute_stop_update(watched_stop);
        }
    }

    _reap_watch_process(watched_stop) {
        if(this._watch_processes !== undefined && this._watch_processes[watched_stop] !== undefined) {
            clearInterval(this._watch_processes[watched_stop]);
            this._watch_processes[watched_stop] = undefined;
        }
    }
    ...
    _execute_stop_update(watched_stop) {
        console.log("[UH] Scheduled new update");
        console.log(watched_stop);
        let link = ETAGrabber._get_link_from_stop(watched_stop.stop_id);
        if(link !== undefined) {
            link.get_arrival_time(watched_stop.stop_id, (err, data) => {
                if(!err && data) {
                    console.log(this);
                    this.emit('stop_update', watched_stop.stop_id, data);
                    console.log("[UH] Emitted stop update");
                } else {
                    console.error(err);
                }
            });
        } else {
            console.warn("[UH] Invalid link sent, kill watch_proc");
            this._reap_watch_process(watched_stop);
        }
    }
    ...
    _handle_update(stop_id, data) {
        if(stop_id in this._watching && this._watching[stop_id].length > 0) {
            ...
            for(watcher of this._watching[stop_id]) {
                ...
                let etaInfo;
                for(etaInfo of data) {
                    if(parseInt(etaInfo.vehicleId) === watcher.bus_id && parseInt(etaInfo.tripNumber) === watcher.trip_id) {
                        ...
                        console.log(etaInfo.etaSeconds + " < " + watcher.time_before + " = " + etaInfo.etaSeconds < watcher.time_before);
                        found = true;
                    }
                }
                if(!found) {
                    //bus has passed, remove watcher
                    console.log("[UH] Reaping watcher for bus pass " + watcher.notification_token);
                    this.remove_watcher(watcher);
                    NotificationHelper.send_bus_missed(watcher.notification_token, watcher.bus_id);
                }
            }
            console.log("[UH] Completed for loop")
        } else {
            //nobody watching, stop the updates
            console.log("[UH] Reaping " + stop_id);
            this._reap_watch_process(stop_id);
        }
    }
}

class NotificationHelper {
    static initialize_firebase() {
        let serviceAccFile = require("../FCMServiceAccount.json");
        admin.initializeApp({
            credential: admin.credential.cert(serviceAccFile),
            databaseURL: "https://nautransit.firebaseio.com"
        });
    }

    static _send_firebase_call(token, data, options) {
        admin.messaging().sendToDevice(token, data, options).catch((error) => {
    ...
    static send_bus_missed(token, bus_id) {
        NotificationHelper._send_firebase_call(token, {
            notification: {
                title: "Bus missed!",
                message: "Whoops! Bus " + bus_id + " arrived much sooner than expected, and it has passed."
            }
        })
    }
